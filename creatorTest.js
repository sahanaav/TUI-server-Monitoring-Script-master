				...
				switch(statusCode) {
					...
					default:
						if (statusCode == 200 && typeof result != undefined) {
							...
							//deferred.reject(err);
							emailSender.sendEmail("Temporary Unavailable ","Creator server is Temprarily unavailable.");
						}
	...
	function callAPIsWithSessionId(sessionId) {
		for (var i = 0; i < listOfApis.length; i++) {
			...
			rest.callAPI(options, function(statusCode,name) {
				...
				switch(statusCode) {
					case 500:
							emailSender.sendEmail("InternalServerErrot(500) with Creator"+name +"API","Internal Error in service or webserver");
						break;
						break;
					default:
						if (statusCode !== 200) {
